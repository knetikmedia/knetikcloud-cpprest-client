...
Payments_AppleApi::Payments_AppleApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<utility::string_t> Payments_AppleApi::verifyAppleReceipt(std::shared_ptr<ApplyPaymentRequest> request)
{


    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/payment/provider/apple/receipt");
    ...
    {
        throw ApiException(400, U("Payments_AppleApi->verifyAppleReceipt does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("Payments_AppleApi->verifyAppleReceipt does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling verifyAppleReceipt: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling verifyAppleReceipt: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
    ...
    .then([=](utility::string_t response)
    {
        ...
        {
            throw ApiException(500
                , U("error calling verifyAppleReceipt: unsupported response type"));
        }
