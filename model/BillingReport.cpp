...
web::json::value BillingReport::toJson() const
{
    ...
    }
    {
        ...
        if(jsonArray.size() > 0)
        {
            val[U("statistics")] = web::json::value::array(jsonArray);
        }
    }
...
void BillingReport::fromJson(web::json::value& val)
{
    ...
    }
    {
        ...
        {
        for( auto& item : val[U("statistics")].as_array() )
        {  
            ...
            m_Statistics.insert(std::pair<utility::string_t,int32_t>( key, ModelBase::int32_tFromJson(item[U("value")])));
        }
        }
    }
}
...
void BillingReport::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
    ...
    }
    {
        ...
        if(jsonArray.size() > 0)
        {
            multipart->add(ModelBase::toHttpContent(namePrefix + U("statistics"), web::json::value::array(jsonArray), U("application/json")));
        }
    }
}
...
void BillingReport::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    ...
    }
    {
        ...
        web::json::value jsonArray = web::json::value::parse(ModelBase::stringFromHttpContent(multipart->getContent(U("statistics"))));
        for( auto& item : jsonArray.as_array() )
        {
            utility::string_t key;
            if(item.has_field(U("key")))
...
void BillingReport::setLastKnownFailures(std::vector<utility::string_t> value)
{
    m_Last_known_failures = value;
    m_Last_known_failuresIsSet = true;
}
bool BillingReport::lastKnownFailuresIsSet() const
{
...
void BillingReport::setStatistics(std::map<utility::string_t, int32_t> value)
{
    m_Statistics = value;
