...
PaymentsGoogleApi::PaymentsGoogleApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<int32_t> PaymentsGoogleApi::handleGooglePayment(std::shared_ptr<GooglePaymentRequest> request)
{


    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/payment/provider/google/payments");
    ...
    {
        throw ApiException(400, U("PaymentsGoogleApi->handleGooglePayment does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("PaymentsGoogleApi->handleGooglePayment does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling handleGooglePayment: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling handleGooglePayment: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
    ...
    .then([=](utility::string_t response)
    {
        ...
        {
            throw ApiException(500
                , U("error calling handleGooglePayment: unsupported response type"));
        }
