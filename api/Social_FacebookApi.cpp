...
Social_FacebookApi::Social_FacebookApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<void> Social_FacebookApi::linkAccounts(std::shared_ptr<FacebookToken> facebookToken)
{


    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/social/facebook/users");
    ...
    {
        throw ApiException(400, U("Social_FacebookApi->linkAccounts does not produce any supported media type"));
    }
    ...
    if ( consumeHttpContentTypes.size() == 0 || consumeHttpContentTypes.find(U("application/json")) != consumeHttpContentTypes.end() )
    {
        ...
        json = ModelBase::toJson(facebookToken);
    ...
    else if( consumeHttpContentTypes.find(U("multipart/form-data")) != consumeHttpContentTypes.end() )
    {
        ...
        std::shared_ptr<MultipartFormData> multipart(new MultipartFormData);
                if(facebookToken.get())
        {
            facebookToken->toMultipart(multipart, U("facebookToken"));
        }
    ...
    {
        throw ApiException(415, U("Social_FacebookApi->linkAccounts does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling linkAccounts: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling linkAccounts: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
