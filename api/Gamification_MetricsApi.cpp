...
#include "Gamification_MetricsApi.h"
#include "IHttpBody.h"
#include "JsonBody.h"
...
Gamification_MetricsApi::Gamification_MetricsApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<void> Gamification_MetricsApi::addMetric(std::shared_ptr<MetricResource> metric)
{


    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/metrics");
    ...
    {
        throw ApiException(400, U("Gamification_MetricsApi->addMetric does not produce any supported media type"));
    }
    ...
    else if( consumeHttpContentTypes.find(U("multipart/form-data")) != consumeHttpContentTypes.end() )
    {
        ...
        std::shared_ptr<MultipartFormData> multipart(new MultipartFormData);
                if(metric.get())
        {
            metric->toMultipart(multipart, U("metric"));
        }
    ...
    {
        throw ApiException(415, U("Gamification_MetricsApi->addMetric does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling addMetric: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling addMetric: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
