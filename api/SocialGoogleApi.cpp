...
SocialGoogleApi::SocialGoogleApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<void> SocialGoogleApi::linkAccounts1(std::shared_ptr<GoogleToken> facebookToken)
{
    

    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/social/google/users");
    ...
    {
        throw ApiException(400, U("SocialGoogleApi->linkAccounts1 does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("SocialGoogleApi->linkAccounts1 does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling linkAccounts1: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling linkAccounts1: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
